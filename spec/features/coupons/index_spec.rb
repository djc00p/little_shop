...
RSpec.describe "Coupon List" do
  ...
  describe "sad path" do
    before :each do
      ...
      create(:order_item, order: @o3, item: @i1, quantity: 4, price: 2)
      create(:order_item, order: @o4, item: @i2, quantity: 5, price: 2)
      @c1, @c2, @c3, @c4 = create_list(:coupon, 4, user: @merchant, active: true)
      @c5 = create(:coupon, user: @merchant, active: false)
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@merchant)
      visit dashboard_path(@merchant)
      click_link "Manage Coupons"
    end

    it "should not be able to add a new coupon if 5 already exist" do
      # save_and_open_page
      expect(page).to_not have_link("Add Coupon")

      visit new_dashboard_coupon_path
      save_and_open_page
      # expect(current_path).to have_content('public/404')
    end
  end
end
